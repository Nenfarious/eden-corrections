...
public class SpamControlManager {
    ...
    private void loadConfiguration() {
        ...
        commandCooldownTime = plugin.getConfigManager().getSpamControlCommandCooldown();
        messageCooldownTime = plugin.getConfigManager().getSpamControlMessageCooldown();
        maxSpamCount = plugin.getConfigManager().getSpamControlMaxSpamCount();
        spamTimeout = plugin.getConfigManager().getSpamControlSpamTimeout();
    }
    ...
    public boolean canExecuteCommand(Player player, String command) {
        ...
        if (lastExecution != null && currentTime - lastExecution < commandCooldownTime) {
            ...
            if (spamCount >= maxSpamCount) {
                plugin.getMessageManager().sendMessage(player, "spam-control.messages.command-spam-warning");
                return false;
            }
            
            plugin.getMessageManager().sendMessage(player, "spam-control.messages.command-cooldown",
                plugin.getMessageManager().stringPlaceholder("command", command));
            return false;
        }
    ...
    public boolean canSendMessage(Player player) {
        ...
        if (lastMessage != null && currentTime - lastMessage < messageCooldownTime) {
            ...
            if (spamCount >= maxSpamCount) {
                plugin.getMessageManager().sendMessage(player, "spam-control.messages.message-spam-warning");
                return false;
            }
            
            plugin.getMessageManager().sendMessage(player, "spam-control.messages.message-cooldown");
            return false;
        }
